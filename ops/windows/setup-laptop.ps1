...
# 2. Scheduled Task — intraday engine. Fires Mon-Fri at 9:10 AM local time.
#    NOTE: assumes laptop clock is set to IST (Asia/Kolkata). Most Indian laptops are.
Write-Host "`n[1/3] Creating Scheduled Task: NSE Intraday Engine (9:10 AM Mon-Fri)..." -ForegroundColor Yellow
...
Write-Host "`n[2/3] Creating Scheduled Task: NSE Dashboard (at logon)..." -ForegroundColor Yellow
...
Write-Host "`n[3/3] Sanity check: Python + key packages..." -ForegroundColor Yellow
...
Write-Host "  - NOTE: Laptop power settings NOT changed. If your laptop sleeps during"
Write-Host "    market hours, the engine pauses. Adjust power plan manually if needed."
Write-Host ""
